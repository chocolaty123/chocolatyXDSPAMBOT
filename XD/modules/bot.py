...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        await e.reply(f"**ᴇʀʀᴏʀ 131**: ѕєяνєя ιѕ яєѕтαятιиg 😈")
        try:
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"» __ωαιт ι αм αɖɖιиg υѕєя ιи ѕυɖσ ℓιѕт...__")
        mks = "SUDO_USER"
        target = ""
        ...
        try:
            ...
            await ok.edit("» 𝙿𝙻𝙴𝙰𝚂𝙴 𝚁𝙴𝙿𝙻𝚈 𝙰 𝚄𝚂𝙴𝚁 !!")
        if len(sudousers) > 0:
            newsudo = f"{sudousers} {target}"
        else:
            newsudo = f"{target}"
        await ok.edit(f"» **ɴᴇᴡ ѕυ𝚍σ υѕєя**: `{target}`\n» `яєѕтαятιиg ʙᴏᴛ...`")
