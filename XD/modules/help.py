...
PythonHelp = f"★ 𝙓𝘿 𝙎𝙥𝙖𝙢 𝙃𝙚𝙡𝙥 𝙈𝙚𝙣𝙪 ★\n\n» **ᴄʟɪᴄᴋ ᴏɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴꜱ ꜰᴏʀ ʜᴇʟᴘ**\n» **ᴅᴇᴠᴇʟᴏᴘᴇʀ: @ROCKY_ISS_BACK**"
...
async def help(event):
    if event.sender_id in SUDO_USERS:
       await event.client.send_file(event.chat_id,
           ...
           [    
            Button.url("• ᴄʜᴀɴɴᴇʟ •", "https://t.me/II_ROCKY_II"),
...
async def helpback(event):
   if event.query.user_id in SUDO_USERS:    
      await event.edit(
        ...
        await event.answer("Make Your Own Altron Bots !! @ROCKY_ISS_BACK", cache_time=0, alert=True)
...
async def help_raid(event):
     if event.query.user_id in SUDO_USERS:
        ...
        await event.answer("Make Your Own XD Bots !! @ROCKY_ISS_BACK", cache_time=0, alert=True)
